...
function[trainError, testError, minTestError, a4, yval] = feedForward_3more_2hid_do(nn,X1,X2,X3,y,X1val,X2val,X3val,yval,meanValues,mean1,dropOutRatios)
...
%     trainError = rms( rms( abs(a4 - y) * 100 ./ repmat(meanValues,size(y,1),1) ) ); 
    ...
    plot( 1:size(a4,1), a4(:,1) ,1:size(a4,1),  yval(:,1) );
    
    testError = mean( mean( abs(a4 - yval) * 100 ./ repmat(meanValues,size(yval,1),1) ) );
    minTestError = min( mean( abs(a4 - yval) * 100 ./ repmat(meanValues,size(yval,1),1) ) );
